...
class SerialSocket(context: Context, device: BluetoothDevice) : BluetoothGattCallback() {
    ...
    private var pairingIntentFilter: IntentFilter? = null
    private var pairingBroadcastReceiver: BroadcastReceiver? = null
    ...
    fun connect(listener: SerialListener?) {
        ...
        context!!.registerReceiver(
            ...
            IntentFilter(Constants.BluetoothService.INTENT_ACTION_DISCONNECT)
        )
    ...
    private fun connectCharacteristics3(gatt: BluetoothGatt) {
        val writeProperties: Int = writeCharacteristic.getProperties()
    ...
    private class MicrochipDelegate : DeviceDelegate() {
        override fun connectCharacteristics(gattService: BluetoothGattService): Boolean {
            ...
            if (writeCharacteristic == null) writeCharacteristic =
                gattService.getCharacteristic(BLUETOOTH_LE_MICROCHIP_CHAR_RW)
            return true
        }
    }

    private class NrfDelegate : DeviceDelegate() {
        override fun connectCharacteristics(gattService: BluetoothGattService): Boolean {
    ...
    private class TelitDelegate : DeviceDelegate() {
